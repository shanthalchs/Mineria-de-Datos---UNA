...
ignore <- clusterEvalQ(clp, {
  ...
  ejecutar.prediccion <- function(datos, formula, muestra, metodo, ...){
    ...
    if(metodo == "lda"){
      browser()
      MC <- table(ttesting[,17], prediccion.lda$class)
    }  
      else {
...
for(i in 1:cantidad.validacion.cruzada) {
  ...
  for(k in 1:cantidad.grupos) {
    ...
    resultado <- lapply(algoritmos, function(ptipos) {
...
ejecutar.prediccion.particular <- function(datos, formula, muestra, metodo) {
  ...
  if(metodo == "lda"){return(ejecutar.prediccion(datos, formula, muestra, metodo))}
}
...
clusterExport(clp, list("tumores", "ejecutar.prediccion", "ejecutar.prediccion.particular"))
...
for(i in 1:cantidad.validacion.cruzada) {
  ...
  for(k in 1:cantidad.grupos) {
    ...
    resultado <- clusterApply(clp, metodos, function(metodo) {
      MC <- ejecutar.prediccion.particular(datos = tumores, formula = tipo~., muestra = muestra, metodo = metodo)
      si.val <- MC["Si", "Si"]
...
for(i in 1:cantidad.validacion.cruzada) {
  ...
  for(k in 1:cantidad.grupos) {
    ...
    resultado <- clusterApply(clp, metodos, function(metodo) {
      MC <- ejecutar.prediccion.particular(datos = datos, formula = BuenPagador~., muestra = muestra, metodo = metodo)
      # Cálculo del ERROR
      error.metodo <- (1-(sum(diag(MC)))/sum(MC))*100
