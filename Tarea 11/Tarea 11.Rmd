...
indices2 <- indices.general(MC.nnet4)
```
...
prediccion.nnet20 <- predict(modelo.nnet20,ttesting)
...
modelo.neuralnet <- train.neuralnet(formula = tipo~., data = taprendizaje,  hidden = c(6,4,2), linear.output = FALSE, threshold = 0.1, stepmax = 1e+06)

prediccion.neuralnet <- predict(modelo.neuralnet, ttesting)
...
rownames(tablaComparativa) <- c("nnet2", "nnet4", "nnet20", "neuralnet")
...
write.csv(tablaComparativa,"Tabla Comparativa tumores3.csv", row.names = FALSE)
...
rownames(tablaComparativa)[15] <- c("nnet")
tablaComparativa
